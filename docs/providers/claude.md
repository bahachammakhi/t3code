...
This guide is for people who want to use more than one Claude setup in B3 Code.
...
In B3 Code Settings, your Claude provider can stay like this:
...
An empty `Claude HOME path` means B3 Code uses your normal home directory.
...
Then add another Claude provider in B3 Code:
...
B3 Code only offers Claude providers that use the same Claude home for an existing thread. A
...
multiple files under its home directory, so B3 Code keeps separate Claude homes isolated instead of
...
Add or edit a Claude provider in B3 Code Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. B3 Code stores the value as a server secret and does not
...
B3 Code does not need a special Claude Code Router provider. Treat the router as a Claude
environment.
...
2. Add or configure a Claude provider in B3 Code.
