...
This guide is for people who want to use more than one Codex account in B3 Code.
...
In B3 Code Settings, name it something obvious:
...
In B3 Code Settings, add another Codex provider:
...
B3 Code shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, B3 Code treats it
